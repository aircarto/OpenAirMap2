    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (pas_de_temps[0] === "2min") {
                filteredData = data.filter(item => item.modele_capteur === "NebuleAir");
                console.log(filteredData);
                console.warn("Uniquement micro-stations NebuleAir pour le pas de temps " + pas_de_temps);
            }
...
function openSidePanel_microStation(data, pas_de_temps_atmo, historique, mesures_atmo) {
    ...
    retreive_historiqueData_microStation(data.id_site, pas_de_temps_atmo, historique, mesures_array);
    ...
    card2_text.innerHTML = "Les micro-stations sont des capteurs de mesure de la qualité de l'air déployés par AtmoSud pour compléter le réseau de stations de référence.";
    ...
    btn_historique_custom.onclick = function (event){
        ...
        if (startDate && startTime && endDate && endTime) {
            ...
            retreive_historiqueData_microStation(data.id_site, pas_de_temps_chart, null, mesures_array,false, startDateTime, endDateTime);
        } else {
...
function retreive_historiqueData_microStation(sensorId, pas_de_temps, historique, mesures_array, add_mesure=false, custom_start=null, custom_end=null) {
    console.log("retreiving data for:", {sensorId:sensorId, pas_de_temps:pas_de_temps, historique:historique, mesures_array:mesures_array, add_mesure:add_mesure});
    ...
    const start_date = custom_start || new Date(Date.now() - (hours * 3600 * 1000)).toISOString();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (pas_de_temps == "2m" || pas_de_temps == "2min" || pas_de_temps == "brute" ) {baseInterval_timeUnit_local = "minute"; baseInterval_count=2}
            if (pas_de_temps == "15m" || pas_de_temps == "qh" || pas_de_temps == "quart-horaire") {baseInterval_timeUnit_local = "minute"; baseInterval_count=15}
            if (pas_de_temps == "1h" || pas_de_temps == "h" || pas_de_temps == "horaire") {baseInterval_timeUnit_local = "hour"; baseInterval_count=1}
            if (pas_de_temps == "24h" || pas_de_temps == "1d" || pas_de_temps == "journalier") {baseInterval_timeUnit_local = "day"; baseInterval_count=1}
            ...
            am5.ready(function() {
                ...
                let chart = amchart_root.container.children.push(am5xy.XYChart.new(amchart_root, {
                    panX: false,
                    panY: false,
                    wheelX: "panX",
                    wheelY: "zoomX",
                ...
                let cursor = chart.set("cursor", am5xy.XYCursor.new(amchart_root, {
                    behavior: "zoomX"
                }));
                cursor.lineY.set("visible", false);
            
                let xAxis = chart.xAxes.push(am5xy.DateAxis.new(amchart_root, {
                    maxDeviation: 0.2,
                    baseInterval: {
                ...
                let yAxis = chart.yAxes.push(am5xy.ValueAxis.new(amchart_root, {
                ...
                Object.keys(seriesData).forEach(variable => {
                    let series = chart.series.push(am5xy.SmoothedXLineSeries.new(amchart_root, {
