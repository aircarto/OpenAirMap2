function formatMapData(data, pas_de_temps, polluant, source) {
    ...
    if (source === "nebuleair") {
        ...
        formattedData = data.map(device => {
            ...
            let paramName = pas_de_temps === "2min" ? mesure_maj : `${mesure_maj}_${pas_de_temps}`;
        ...
        formattedData = data.map(device => {
            return {
                ...
                valeur_corrige: device.valeur !== null ? parseFloat(device.valeur) : false,
                source: "atmosud_micro",
                mesure: device.variable ? device.variable.toLowerCase() : polluant,
                pas_de_temps: device.pas_de_temps ? device.pas_de_temps.toString() : pas_de_temps,
                plage_historique: "24h", // Default value, can be adjusted
                ...
                rawDeviceData: device, // Store original data for side panel
                openSidePanelFunction: (rawData, pdt, timespan, mes) => {
                    openSidePanel_microStation(rawData, pdt, timespan, mes);
                }
            };
        });
        ...
        if (pas_de_temps === "2min") {
            formattedData = formattedData.filter(item => item.modele === "NebuleAir");
        }
    }
    ...
    formattedData.forEach(device => {
        ...
        if (device.connected) {
            ...
            if (device.mesure === "pm1" || device.mesure === "pm25" || device.mesure === "pm2.5") {
...
function displayFormattedDataOnMap(formattedData, layer) {
    ...
    formattedData.forEach(device => {
        ...
        let deviceMarker = L.marker([device.latitude, device.longitude], { icon: marker_icon })
            .addTo(layer);
        ...
        var text_param = L.divIcon({
            ...
            html: '<div id="textDiv" style="font-size: ' + device.textSize + 'px;">' + roundedValue + '</div>',
            iconAnchor: [device.textXPosition, device.textYPosition]
        });
        
        let textMarker = L.marker([device.latitude, device.longitude], { icon: text_param })
            .on('click', function() {
                // Handle marker selection
                if (globalSelectedMarker && globalSelectedMarker !== deviceMarker) {
                ...
                if (device.source === "nebuleair") {
                    openSidePanel_nebuleAir(device.rawDeviceData, device.pas_de_temps, device.plage_historique, device.mesure);
                } else if (device.source === "atmosud_micro") {
                    openSidePanel_microStation(device.rawDeviceData, device.pas_de_temps, device.plage_historique, device.mesure);
                }
            })
            .addTo(layer);
        ...
        function highlightMarker() {
            ...
            deviceInfo._div.querySelector('#device-name').textContent = device.id;
            deviceInfo._div.querySelector('#device-details').textContent = `Type: ${device.modele}`;
        ...
        deviceMarker.on('mouseover', highlightMarker).on('mouseout', resetMarker);
